...
class TaskExecutionSystem:
    ...
    def generate_execution_plan(self, query: str) -> List[Dict]:
        ...
        response = self.llm_client.chat_completion(messages, debug=self.debug)
    ...
    def execute_sub_step(self, sub_step: Dict, prev_step_result: Optional[str] = None) -> Dict:
        """
        执行单个子步骤
        ...
        if action == 'retrieval':
            ...
            return {
                'status': 'success',
                'data': results,
                'message': f"检索到 {len(results) if results else 0} 条结果"
            }
            ...
            if self.debug:
                print(f"[DEBUG] LLM生成 - 提示: {prompt[:200]}...")

            result = self.llm_client.chat_completion(messages, debug=self.debug)
            ...
            if self.debug:
                print(f"[DEBUG] LLM生成结果: {content[:200]}...")
    ...
    def execute_step(self, step: Dict, prev_step_result: Optional[str] = None) -> Dict:
        """
        执行单个步骤（包含多个子步骤）
        ...
        for sub_step in sub_steps:
            ...
            # 执行子步骤
            result = self.execute_sub_step(sub_step, prev_step_result)
    ...
    def execute_task(self, query: str) -> Dict:
        ...
        for i, step in enumerate(execution_plan):
            step_result = self.execute_step(step, prev_step_result)
        ...
        # 3. 生成最终答案
        print("\n正在生成最终答案...")
        final_answer = self.generate_final_answer(query, all_results)
    ...
    def generate_final_answer(self, original_query: str, all_results: Dict) -> str:
        """
        根据所有步骤结果生成最终答案
        ...
        for step_id, step_result in all_results.items():
            context_parts.append(f"步骤 {step_id} 结果: {step_result['combined_result']}")

        context = "\n".join(context_parts)

        final_prompt = f"""
        基于以下任务执行结果，为原始查询生成最终答案：
        ...
        if self.debug:
            print(f"[DEBUG] 生成最终答案的输入: {final_prompt[:200]}...")
        ...
        try:
            ...
            if self.debug:
                print(f"[DEBUG] 生成最终答案的输出: {content[:500]}...")
